...
% \def\besta{\hat{\tilde{\mathbf a}}}
	...
	\begin{theorem}
		...
		\begin{align*}
			...
			\hat{\tilde{\mathbf a}} &= (\hat a_i \cond i\in \hat I_\lams)^T \text{--- ненулевые компоненты $\hat{\mb a}$,} \\
			\tilde{\mb x} &= (x_i \cond i\in \hat I_\lams)^T,
			\tilde{\mb X} = (\tilde{\mb x}_1, \ldots, \tilde{\mb x}_N)^T \text{--- значения признаков $\hat I_\lams,$} \\
			\hat y_k &= \hat{\tilde{\mathbf a}}^T\!(\Omega^*)\tilde{\mb x}_k.
			% \text{--- ответы алгоритма, обучишегося на всём $\Omega^*$,} \\
		\end{align*}
	\end{theorem}
\end{frame}
