...
@variant.text(u'''
    Два конденсатора ёмкостей ${C1:Task}$ и ${C2:Task}$ последовательно подключают
...
@variant.text(u'''
    Лампочки, сопротивления которых ${R1:Task}$ и ${R2:Task}$, поочерёдно подключённные к некоторому источнику тока,
