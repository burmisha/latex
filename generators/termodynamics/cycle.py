...
@variant.answer_align([
    ...
    '\\eta_\\text{Карно} &= 1 - \\frac{T_\\text{х}}{T_\\text{н}} = 1 - \\frac{T_4}{T_2}'
...
@variant.answer_align([
    ...
    '\\eta_\\text{Карно} &= 1 - \\frac{T_\\text{х}}{T_\\text{н}} = 1 - \\frac{T_1}{T_3}'
...
@variant.answer_tex('''
    ...
    Q
        ...
        &= \\frac{\\Delta V}2 * \\cbr{5 * \\cbr{{a1}P_0 - \\frac{P_0}{V_0} V} - \\frac{P_0}{V_0} V}
         = \\frac{\\Delta V * P_0}2 * \\cbr{5 * {a1} - 8\\fracV{V_0}}.
    \\end{align*}
    ...
    \\begin{itemize}
        ...
        \\item если $5 * {a1} - 8\\fracV{V_0} > 0$ — тепло подводят, если же меньше нуля — отводят.
    \\end{itemize}
