...
@variant.arg(alpha=[u'4', u'16', u'25'])
...
@variant.arg(mLetter=[u'm', u'M'])
@variant.arg(mValue=[100, 200, 250, 400])
@variant.arg(v=[u'v = %d м / с' % v for v in [1, 2, 4, 5]])
...
@variant.arg(lmbd=[u'\\lambda = %d м' % lmbd for lmbd in [3, 4, 5, 6]])
...
@variant.arg(lmbd=[u'\\lambda = %.1f м' % lmbd for lmbd in [1.2, 1.5, 2.1, 2.4]])
@variant.arg(T=[u'T = %d мc' % T for T in [2, 3, 4, 5, 6]])
...
@variant.arg(N=[u'N = %d' % N for N in [4, 5, 6]])
@variant.arg(t=[u't = %d c' % t for t in [5, 6, 8, 10]])
@variant.arg(v=[u'v = %d м / с' % v for v in [1, 2, 3, 4, 5]])
...
@variant.arg(l=[u'l = %d см' % l for l in [20, 25, 40, 50, 75]])
