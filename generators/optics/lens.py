...
# @variant.text('''
#     Предмет высотой й = 40 см находится на расстоянии 4=1 м
#     от вертикально расположеииой рассеивающей линзы с фокусным расстоянием Ё=-25 см.
#     Где находится изображение предмета? Определить высоту изображения, = Fr 2M,
#     изображение мнимое; Н = | -0д8м }
# ''')
# @variant.answer_short('')
...
# @variant.text('''
#     На каком расстоянии от двояковыпуклой линзы с оптической силой О = 2,5 дитр
#     надо поместить предмет, чтобы его изображение получилось иа расстоянии /= 2 м от линзы 7?
# ''')
# @variant.answer_short('')
...
@variant.text('''
    На экране, расположенном иа расстоянии {b:V:e} от собирающей линзы,
...
@variant.text('''
    ...
    с увеличением г;= 2. Предмет передвинули на | см. Для того чтобы
    получить резкое изображение, пришлось передвинуть экран.
    При этом увеличение оказалось равным 7= 4. На какое расстояние
    пришлось передвинуть экран? [ На см ]
''')
@variant.answer_short('')
...
@variant.text('''
    Тонкая собирающая линза дает изображение предмета на экране высотой Н,
    и Н, при двух положениях линзы между предметом и экраном,
    расстояние между ними неизменно. Чему равна высота предмета А? [а= Н.Н, ] :
''')
@variant.answer_short('')
...
@variant.text('''
    ...
    если разрешающая способность пленки 6=0,01 мм? Каким должно быть
    время экспозиции & чтобы полностью использовать возможности пленки?
    Фокусиое расстояние объектива используемого фотоаппарата Р=10 см, высота орбиты’ спутника Н=100 км.
''')
@variant.answer_short('')
...
@variant.text('''
    ...
    имеет фокусиое расстояние Р’= 8 см. Разрешающая способность пленки д = 10? мм.
    На какой высоте должен лететь самолет, чтобы иа фотографии можно
    было различить листья деревьев размером |=5 см? При какой скорости самолета изображение ие будет размытым, если время зкспозиции 1=10`? с?
''')
@variant.answer_short('')
