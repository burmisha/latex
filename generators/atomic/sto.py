...
from generators.helpers import Consts, Decimal, n_times, UnitValue
...
@variant.text('''
    Как (и на сколько) изменяется масса {m:V:e} воды при {what}?
''')
@variant.solution_space(80)
@variant.arg(m='1/50/200/1000/2500 т')
@variant.arg(what=[
    'нагревании от $0\\celsius$ до $100\\celsius$',
    'испарении при $100\\celsius$',
    'замерзании при $0\\celsius$',
])
@variant.answer_short('{dm:L} = \\frac Q{c^2} \\approx {dm:V}')
@variant.is_one_arg
class Gelfgat_9_9(variant.VariantTask):
    def GetUpdateOneArg(self, a):
        if a.what == 'нагревании от $0\\celsius$ до $100\\celsius$':
            Q = Consts.water.c * a.m * UnitValue('100 К')
        elif a.what == 'испарении при $100\\celsius$':
            Q = Consts.water.L * a.m
        elif a.what == 'замерзании при $0\\celsius$':
            Q = Consts.water.lmbd * a.m * (-1)
        else:
            raise RuntimeError(f'Invalid what: {a.what}')

        return dict(
            dm=(Q / Consts.c / Consts.c).SetLetter('\\Delta m'),
        )
