...
% Тогда из ограничений исходной задачи очевидным образом получаем:
% $$\xi_i = \norm{\mb x_i - \mb a}^2,$$ 
% а задача приобретает вид 
% $$С\suml_i\norm{\mb x_i - \mb a}^2 \to \min\limits_{\mb a}.$$
% Решение этой задачи: $$\mb a = \ol{\mb x} = \frac{\suml_i \mb x_i}N.$$

% Простым способом определить, какой же случай из двух вышеописанных выполнен, является сравнение двух значений функионалов.
