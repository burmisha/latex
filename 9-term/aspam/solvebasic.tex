...
% \al{
% 	\Ell(\mb a, R,\mb\xi,\mb\alpha,\mb\gamma)
% 	% =  \suml_i\ai\mbx i\cd\mbx i-\suml_{i,j}\ai\aj\mbx j\cd\mbx i \to \max_{\mb\alpha}. !!! NO
% 	&=	\suml_i\ai\mbx i\cd\mbx i - 2\suml_i\ai\mbx i\cd\frac{\suml_j\aj\mbx j}{\suml_j\aj} + \suml_i\ai\frac{\suml_j\aj\mbx j}{\suml_j\aj}\cd\frac{\suml_j\aj\mbx j}{\suml_j\aj} = \\
% 	&=\suml_i\ai\mbx i\cd\mbx i - \frac1{\suml_j\aj}\suml_{i,j}\ai\aj\mbx i\cd\mbx j.
% }
% }

% Решая задачу максимизации, но уже без ограничения $\sum_i\alpha_i = 1,$ аналогично получим вектор $\alpha$, 
% по нему сможем восстановим центр сферы: $\mb a = \frac{\sum_i\alpha_i\mb x_i}{\sum_i \alpha_i}.$ 
% Теперь, определив точки лежащие на сфере, вычислим $R$ и проверим, действительно ли получен $0$. 
% Если окажется, что это так, то следует выбрать именно этот вариант, поскольку множество, по которому производится максимизация $\Ell$, шире.
